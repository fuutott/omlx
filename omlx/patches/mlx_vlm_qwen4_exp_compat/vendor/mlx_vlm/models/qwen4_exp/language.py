...
class _SafeTensorMMap:
    ...
    def rows_np(self, key: str, rows) -> tuple[np.ndarray, str]:
        """Upstream prefetch API backed by the shared owned-row reader."""
        return self.rows_numpy(key, rows), self.tensor_dtype(key)

    def rows_numpy(self, key: str, rows) -> np.ndarray:
        """Owned rows; also used by the fork's portable reader tests/tools."""
        ...
        if row_indices.size == 0:
            return np.empty((0, shape[1]), dtype=np_dtype)
        gather_start = None
    ...
    # Historical helper name retained for the portable reader/benchmark API.
    to_mlx = to_mx
...
class DiskBackedShardedEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self._prefix = prefix
        # Upstream batches/uploads and prefetches by default. Preserve the
        # fork's explicit reference-path switch for controlled comparisons.
        self.batched_gather = os.environ.get("OMLX_QWEN4_PLE_BATCHED_GATHER", "1") == "1"
        self.rows_read = 0
        self.last_uploads = 0
