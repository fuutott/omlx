...
@pytest.mark.skipif(not mx.metal.is_available(), reason="requires Metal")
@pytest.mark.parametrize("bits", [4, 5, 6, 8])
def test_fused_mixed_injection_bits_with_lazy_strided_batch(bits):
    from mlx_vlm.models.qwen4_exp import hc_fused

    mx.random.seed(100 + bits)
    module = _module(4)
    module.block_inject_weight = _module(bits).block_inject_weight
    # Do not eval the producer: exercise contiguous handling of a lazy view.
    x = mx.random.normal((2, 8, WIDTH)).astype(mx.bfloat16)[:, ::2, :]
    assert hc_fused.compatible(module, x)
    actual = hc_fused.fused_forward(module, x)
